(ns propagators.infra.datastructures.compound_subnet
  "Compound linked-list subnet state and merge (no propagators, no scheduler)."
  (:require [propagators.infra.cells.avatar :as avatar]
            [propagators.infra.cells.cell :as cell]
            [propagators.infra.cells.snapshot :refer [pop-inputs]]
            [propagators.infra.datastructures.compound_strongest_result :as strongest]
            ...
            [propagators.infra.ids :as id]
            [propagators.infra.network :as net]
            [propagators.infra.propagator :as prop]
            [propagators.infra.stdlib.effect :refer [hook-output-taps]]))
...
(defn run-subnet-effectful
  ...
  [state]
  (let [run-tasks (requiring-resolve 'propagators.infra.core/run-tasks)
...
(defn- merge-entry
  ...
  (let [merge-cell (requiring-resolve 'propagators.infra.cells.merge/cell-merge)
        strongest-value (requiring-resolve 'propagators.infra.cells.merge/strongest-value)
