(ns propagators.infra.datastructures.named-network
  ...
  (:require [clojure.set :as set]
            [propagators.infra.cells.bool4 :as b]
...
(defn- scope-source-content?
  ...
  (boolean
   (when-let [scope-content?
              (requiring-resolve
               'propagators.infra.datastructures.scope-source/scope-content?)]
     (scope-content? x))))
...
(defn- merge-scope-source-entry
  ...
  (let [a-content (cell/cell-content a-entry)
    ...
    (when (and (scope-source-mergeable? a-content)
      ...
      (let [merge-content
            (requiring-resolve
             'propagators.infra.datastructures.scope-source/merge-content)
            strongest-value
            (requiring-resolve
             'propagators.infra.datastructures.scope-source/strongest-value)
