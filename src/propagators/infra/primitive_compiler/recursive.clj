(ns propagators.infra.primitive-compiler.recursive)

(defn recursive-closure
  "Compile-facing entry point for source-level GUR recursive closures.

  The implementation stays in `propagators.infra.gur.accumulating.source`; resolving it
  lazily keeps this namespace from depending on the experiment namespace during
  compiler load.
  "
  ([name params body]
   ((requiring-resolve 'propagators.infra.gur.accumulating.source/source-recursive-closure)
    name
    params
    body))
