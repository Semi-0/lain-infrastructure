(ns propagators.infra.network-vm
  "Experimental monotone network VM facade.

  This namespace is intentionally separate from `propagators.infra.gur`; it is a
  parallel experiment, not the canonical GUR implementation."
  (:require [propagators.infra.network-vm.executor :as executor]
            [propagators.infra.network-vm.instructions :as instructions]))
