(ns propagators.infra.propagator
  (:require [as-messages :refer [as-messages]]
            ...
            [propagators.infra.network :refer [as-net
                                         assoc-net-prop
                                         net-graph
                                         net-with-graph
                                         network-env-lookup]]))
...
(defn construct-propagator
  ...
  ([id-or-name activate inputs outputs]
   (if (propagators.infra.ids/node-id? id-or-name)
...
(defn compound-propagator
  ...
  Activation logic lives in `propagators.infra.closure/compound-activate`."
  [closure-in inputs outputs]
  (let [activate (requiring-resolve 'propagators.infra.closure/compound-activate)]
