(ns propagators.infra.cells.merge
  ...
  (:require [propagators.infra.cells.cell :as cell]
            ...
            [propagators.infra.datastructures.compound_subnet :as subnet]
            [propagators.infra.datastructures.compound_subnet_state :as state]
            [propagators.infra.datastructures.compound_update :as update]
            [propagators.infra.datastructures.evidence-set :as evidence]
            [propagators.infra.datastructures.named-network :as named]
            [propagators.infra.network :as net]
            [propagators.infra.datastructures.reducer-cell :as reducer-cell]
            [propagators.infra.datastructures.reducer-subnet :as reducer]))
...
(def ^:private semantic-kind-fn
  (delay
    (requiring-resolve
     'propagators.infra.datastructures.compound-information/semantic-kind)))
...
(def ^:private event-merge-fn
  (resolved 'propagators.infra.datastructures.event/merge-content))
(def ^:private event-strongest-fn
  (resolved 'propagators.infra.datastructures.event/strongest-value))
(def ^:private behavior-merge-fn
  (resolved 'propagators.infra.datastructures.behavior/merge-content))
(def ^:private behavior-strongest-fn
  (resolved 'propagators.infra.datastructures.behavior/strongest-value))
(def ^:private tms-merge-fn
  (resolved 'propagators.infra.datastructures.tms/merge-distributed-content))
(def ^:private tms-strongest-fn
  (resolved 'propagators.infra.datastructures.tms/strongest-distributed-value))
...
(defn- network-vm-nested-delta?
  [x]
  (and (map? x)
       (= "propagators.infra.network_vm.nested.NetworkDelta"
          (.getName (class x)))))
  ...
  (fn [content update _network]
    (or
     ...
     (cond
      ...
      (or ((requiring-resolve 'propagators.infra.semantic-trace/semantic-trace-graph?) content)
          ((requiring-resolve 'propagators.infra.semantic-trace/semantic-trace-graph?) update))
      :semantic-trace-graph
      (or ((requiring-resolve 'propagators.infra.semantic-trace/epoch?) content)
          ((requiring-resolve 'propagators.infra.semantic-trace/epoch?) update))
...
(defn- accessor-network-update?
  ...
  (and (named/named-network? update)
       (let [accessor-network? (requiring-resolve
                                'propagators.infra.datastructures.compound-object.merge/accessor-network?)]
...
(defn- normalize-named-network-content
  ...
  (if (or (value/nothing? content)
    ...
    content
    ((requiring-resolve
      'propagators.infra.datastructures.compound-object/compound-object)
     content)))

(defn- merge-accessor-network-content
  [content update]
  ((requiring-resolve
    'propagators.infra.datastructures.compound-object.merge/merge-named-network-content)
   content
   update))
...
(defmethod built-in-cell-merge :network-vm-nested-delta
  [content update _network]
  ((requiring-resolve
    'propagators.infra.network-vm.nested/merge-network-delta-content)
   content
   update))

(defmethod built-in-cell-merge :semantic-trace-graph
  [content update _network]
  ((requiring-resolve 'propagators.infra.semantic-trace/graph-union)
   content
   update))

(defmethod built-in-cell-merge :semantic-trace-epoch
  [content update _network]
  ((requiring-resolve 'propagators.infra.semantic-trace/merge-epoch)
   content
   update))
...
(defn- protocol-handled?
  [result]
  (and (map? result)
       (true? (get result :propagators.infra.cells.cell-protocol/handled?))))

(defn- protocol-handled-value
  [result]
  (get result :propagators.infra.cells.cell-protocol/value))

(defn- protocol-cell-merge
  [content update network]
  (let [try-cell-merge
        (requiring-resolve
         'propagators.infra.cells.cell-protocol/try-cell-merge)]
...
(defn- protocol-cell-strongest
  ...
  (let [try-cell-strongest
        (requiring-resolve
         'propagators.infra.cells.cell-protocol/try-cell-strongest)]
