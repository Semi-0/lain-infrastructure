(ns propagators.infra.gur.subenv.dispatch
...
(defn- eval-child-local
  [msg child-net]
  (let [eval-cell (requiring-resolve 'propagators.infra.core/eval-cell)
...
(defn- store-child-network
  ...
  (if (accumulating-gur-network? child-net)
    (eval-cell owner-id
               (message owner-id
                        ((requiring-resolve
                          'propagators.infra.gur.accumulating/add-task-facts)
                         child-net
                         task-cause
                         tasks
                         task-index))
