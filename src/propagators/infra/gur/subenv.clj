(ns propagators.infra.gur.subenv
  "Compatibility facade for the deprecated lexical sub-env GUR experiment.

  New compiler/macro work should use `propagators.infra.gur`, which points at
  ...
  The implementation is split by responsibility under `propagators.infra.gur.subenv.*`:
  env/dispatch-directory, child queues, output projection, frame application,
  and concrete example probes.
  "
  (:require [propagators.infra.gur.subenv.dispatch :as dispatch]
            [propagators.infra.gur.subenv.env :as env]
            [propagators.infra.gur.subenv.examples :as examples]
            [propagators.infra.gur.subenv.frame :as frame]
            [propagators.infra.gur.subenv.output :as output]
            [propagators.infra.gur.subenv.queue :as queue]
            [propagators.infra.gur.subenv.source :as source]))
