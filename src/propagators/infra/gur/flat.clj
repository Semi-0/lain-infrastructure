(ns propagators.infra.gur.flat
  ...
  (:require [propagators.infra.application :as app]
            [propagators.infra.cells.value :as value]
            [propagators.infra.gur.flat.effects :as effects]
            [propagators.infra.message :refer [message]]
            [propagators.infra.network :as net]
            [propagators.infra.network-vm.instructions :as instr]))
...
(defn- install-context
  [network app-key]
  ((requiring-resolve 'propagators.infra.install/context) network app-key))

(defn- install-result
  [ctx]
  ((requiring-resolve 'propagators.infra.install/result) ctx))

(defn recursive-declaration
  "Recursive closure whose body writes through `propagators.infra.install`.
