...
**Integration point in this repo:** the hook is **`propagators.infra.cells.merge/cell-merge`** (and related `merge-cell-entry` / `strongest-value` policy), called from `eval-cell` when a message is absorbed:
...
`propagators.infra.datastructures.tms` is a reducer-cell experiment that projects TMS
...
| **Stdlib** | Leaf namespaces under `propagators.infra.stdlib.*`; deprecated barrel |
