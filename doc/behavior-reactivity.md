...
`propagators.infra.datastructures.reducer-cell` is a parallel, smaller reducer value
...
`propagators.infra.datastructures.tms.core` contains the shared TMS fact and
projection model. `propagators.infra.datastructures.tms.distributed` is the compiler-2
default path. `propagators.infra.datastructures.tms.legacy` keeps centralized
...
The `:support/source` value may be a `propagators.infra.scoped-address/name-ref`.
...
- compiler-2 now keeps compiler-facing TMS operators in
  `propagators.compiler.operators.tms`, behavior operators in
  `propagators.compiler.operators.behavior`, and centralized compatibility operators in
  `propagators.compiler.legacy`;
- `propagators.compiler.main/compile-source-with-behavior-tms` compiles with
...
`propagators.infra.datastructures.behavior-algebra` defines pure operations over
...
`propagators.infra.stdlib.arithmetic.behavior` provides behavior-history arithmetic in
...
propagators.infra. That means compiled and hand-wired behavior arithmetic share the
...
`propagators.runtime.tui.annotations` projects the displayed value exactly
...
```sh
clojure -M:test propagators.infra.behavior-algebra-test
clojure -M:test propagators.infra.behavior-arithmetic-test
clojure -M:test propagators.infra.behavior-compiler-test
clojure -M:test propagators.infra.behavior-test
clojure -M:test propagators.infra.compile-2-test
clojure -M:test propagators.infra.tms-test propagators.infra.reducer-cell-test
clojure -M:test
```
