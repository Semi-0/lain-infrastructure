...
Stage 1 now uses a **single global policy** in `propagators.infra.builder-policy` (not per-install `run-tasks`):
...
**`:queue` does not run the scheduler on each install.** It only accumulates tasks; `propagators.infra.compile/flush-queued-tasks` drains the queue once per flush boundary.
...
- `propagators.infra.builder-policy/*builder-policy*` — `:lazy` | `:queue`
- `propagators.infra.network-builder/install-propagator*`, `seed-cell*`, `run-queued-tasks`
- `propagators.infra.compile` — compile ctx carries `:tasks`; flush at end of `do` and `eval-net*`
...
```clojure
;; conceptual API in propagators.infra.stdlib.arithmetic or propagators.infra.layered
...
Default: **idempotent** + single install at definition site; matches “installer runs once” without removing props from propagators.tui.graph.
...
- `p:layered-procedure` + extension cell when the layer is **driven** from elsewhere in the propagators.tui.graph.
