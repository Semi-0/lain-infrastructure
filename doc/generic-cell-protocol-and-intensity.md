...
`propagators.infra.cells.merge` keeps the old built-in behavior as fallback helpers:
...
`propagators.infra.stdlib.arithmetic.intensity` defines an arithmetic layer closure
...
`propagators.infra.debugger` is opt-in. Generic application reports:
