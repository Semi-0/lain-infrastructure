...
`propagators.infra.gur` implementation. It declares recursive topology directly
in the active `Net`. The accumulating-GUR material below documents the explicit
`propagators.infra.gur.accumulating` alternative and historical measurements.
See [Flat GUR Default](flat-gur-default.md).
...
implementation is split into a thin `propagators.infra.gur.subenv` facade plus focused
...
- `cca0b94 Add routed GUR experiment` added `:apply-topology-installer`, an
  ...
  enqueues the installed propagators.infra.
- The current lexical sub-env GUR experiment adds `core/eval-cell*` as the
  ...
  `propagators.infra.gur.subenv` / `propagators.infra.gur.subenv.dispatch`, so the core
...
network value. A later caller chooses when to enqueue the declared propagators.infra.
...
### Experiment H: Lexical Sub-Env GUR In `propagators.infra.gur.subenv`
...
Contextual recursion is represented as ordinary propagators.infra. The core
...
local source macro. The public `propagators.infra.compile/def-recursive` facade now
...
`::*`, and `::quot` resolves to `propagators.infra.stdlib.prop`, while `::apply` and
...
Evidence: `propagators.infra.gur-subenv-test` covers:
...
into `propagators.infra.core`. The later compiler-2/GUR linked-list probe is
...
### Experiment I: Main Accumulating GUR In `propagators.infra.gur`
...
Evidence: `propagators.infra.gur-accumulating-test` covers:
...
- Correctness after this follow-up:
  `clojure -M:test propagators.infra.gur-accumulating-test` passed twice, and
...
- Correctness after the guard:
  `clojure -M:test propagators.infra.gur-accumulating-test`,
  `clojure -M:test propagators.infra.gur-subenv-test propagators.infra.recursive-compound-test propagators.compiler-gur-linked-list-test propagators.infra.linked-list-access-test`,
...
- Focused correctness after this change:
  `clojure -M:test propagators.infra.compound-object-network-slot-test
  propagators.infra.compound-object-test propagators.infra.gur-accumulating-test`, and full
...
- Diagnostic command:
  `clojure -M -m propagators.tui.graph.gur-mapper-topology-draw 15 15
...
- A more aggressive direct accumulated-fragment join was tested first and
  rejected. It made `clojure -M:test propagators.infra.gur-accumulating-test` run for
...
- Regression evidence:
  ...
  ```bash
  clojure -M:test propagators.infra.named-network-test propagators.infra.gur-accumulating-test
  clojure -M:test
  ```
...
Decision: make this the main GUR path. `propagators.infra.gur` now re-exports the
accumulating implementation, and `propagators.infra.compile/def-recursive` builds
...
`propagators.tui.graph.vijual` stress-majorized directed layout from the real late-cdr accessor
...
- synchronization back through the continuation tunnel without changing the
  kernel or splicing branch declarations into the parent propagators.tui.graph.
...
- Successful recursive computation is composed out of propagators.infra. The
...
- It does not install the expanded network into the outer propagators.tui.graph.
...
the current `propagators.infra.gur` namespace now means the accumulating GUR facade.
...
`propagators.infra.gur.subenv` is the deprecated generalized-subenv experiment. It keeps
...
The implementation is split under `propagators.infra.gur.subenv.*`, with the top-level
...
It still does not mutate the live outer propagators.tui.graph. The result is a network value,
...
That cursor path has now been moved under `propagators.infra.deprecated.cursor`.
...
- Dynamic nested map uses per-leaf temporary network runs; declared nested map
  ...
  output accessors, then lets the caller run the propagators.tui.graph.
...
9. `2026-06-17` Lexical sub-env GUR with contextual apply/recur.
   ...
   propagators.infra. `reduce-list` sums `[0..5]` to `15`,
...
10. `2026-06-17` Frame-publisher accessor export for lexical sub-env GUR.
    ...
    Evidence: `propagators.infra.gur-subenv-test` includes a regression that redefines
...
12. `2026-06-21` Parallel compiler-2/GUR linked-list declaration probe.
    ...
    Outcome: `propagators.compiler-gur-linked-list-test` builds
...
13. `2026-06-22` Parallel accumulating GUR with one owner cell.
    ...
    frame owner cells.
    Outcome: `propagators.infra.gur.accumulating` adds deterministic frame fragments,
...
These ASCII graphs were generated with `propagators.tui.graph.vijual` from real network values,
...
outer nested accessor propagators.infra. The inner network must either project a
...
Focused evidence from `propagators.infra.compound-object-network-slot-test`:
