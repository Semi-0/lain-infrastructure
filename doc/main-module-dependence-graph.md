...
| Vijual tooling | `graph/vijual.clj`, `graph/vijual/layout/*` | graph/layout/render only | Analysis renderer for dependency graphs, not a runtime dependency of propagators.infra. |

The current non-uniformity is visible in the imports:

- `propagators.infra.layered` depends on `propagators.infra.application`,
  `propagators.infra.dispatch`, and `propagators.infra.datastructures.compound-object`.
- `propagators.infra.generic-procedure.application` depends on the same application,
...
`propagators.tui.graph.vijual/draw-stress-directed-graph`.
...
`propagators.infra.gur/p:apply-closure` now has the right location in the graph: it is
