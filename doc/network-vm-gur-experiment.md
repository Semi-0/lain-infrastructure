...
The tiny `propagators.infra.network-vm.gur` layer treats GUR concepts as macros over
VM instructions:
...
The scalar baseline stays in `propagators.infra.network-vm.executor`. The nested VM
experiment lives separately in `propagators.infra.network-vm.nested` and
`propagators.infra.network-vm.nested.gur`. Neither replaces accumulating GUR, neither
re-exports through `propagators.infra.gur`, and neither changes `propagators.infra.core` or
cell merge.
...
`propagators.infra.network-vm.nested.gur` is intentionally tiny. A recursive closure
...
Focused result from `propagators.infra.network-vm-nested-test`:
...
`propagators.infra.gur.flat` is a smaller parallel experiment. It removes the
...
The recursive layer in `propagators.infra.gur.flat` keeps the same
...
`propagators.infra.core` scheduler applies declaration effects and schedules newly
installed props.
...
Focused flat-effect result from `propagators.infra.network-vm-flat-test`:
...
`propagators.infra.install` is now the preferred authoring surface for flat
...
```bash
clojure -M:test propagators.infra.network-vm-flat-test
clojure -M:test propagators.infra.network-vm-test
clojure -M:test propagators.infra.network-vm-nested-test
```
