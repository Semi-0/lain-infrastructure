...
`propagators.infra.layered`. Defaults are represented as ordinary procedure slots.
...
uses `propagators.infra.datastructures.compound-object/p:reduce`: it gathers named
result-bank slots through `p:slot`/`p:layer`, then runs a supplied reducer
network over the gathered slot cells. `propagators.infra.dispatch/reduce-results`
...
`propagators.infra.dispatch` contains reusable topology helpers:
...
```clojure
(require '[propagators.infra.stdlib.provenance-arithmetic :as prov-arith])
...
- **Default:** `propagators.infra.stdlib.provenance-arithmetic` (`+`, `-`, `*`, `/` on a
