...
- [Accumulating GUR](accumulating-gur.md) explains the current main GUR
  implementation, from `propagators.infra.gur` through the accumulating submodules and
  runner.
...
- [Recursive Compound Propagator](recursive-compound-propagator.md) describes
  ...
  Fibonacci proof.
- [Main Module Dependence Graph](main-module-dependence-propagators.tui.graph.md) maps the
...
- `propagators.infra.stdlib.provenance-arithmetic` — `+`, `-`, `*`, `/` that bootstrap
...
`propagators.infra.gur`, which re-exports the accumulating implementation; older
